 ...
 * AI inspector panel.
 *
 * Two modes share this panel:
 * - ``note`` (default, M6): the existing current-note actions (ask / summarize /
 *   tags / related) — behaviour unchanged;
 * - ``vault`` (M14): the knowledge-base question box, which retrieves from the
 *   whole Vault, answers with grounded citations and lists clickable sources.
 */
export function AIPanel({onClose,onOpenNote,enabled=true,revision,onManageProviders,onSwitched}:{onClose:()=>void;onOpenNote?:(path:string)=>void;enabled?:boolean;revision?:number;onManageProviders?:()=>void;onSwitched?:(settings:ServiceSettings)=>void}) {
  ...
  const [mode,setMode]=useState<"note"|"vault">("note");
