  ...
  return <div className={`workspace ${s.theme}`} style={{"--sidebar-width": `${s.sidebarWidth}px`, "--inspector-width": `${s.inspectorWidth}px`} as CSSProperties}>
    ...
    <main className="workspace-main"><header className="workspace-header">{!leftOpen && <IconButton onClick={toggleLeft} aria-label={tr("显示文件侧栏", "Show file sidebar")} title={tr("显示文件侧栏", "Show file sidebar")}><Icon name="panelLeft" size={17}/></IconButton>}<TabBar tabs={s.tabs} active={s.activePath} sessions={s.sessions} onActivate={activate} onClose={close} onRetry={path => void s.save(path, "manual")}/>{!s.tabs.length && <span className="workspace-caption">{tr("工作空间", "Workspace")}</span>}<IconButton className="inspector-toggle" onClick={onToggleInspector} aria-label={tr("切换辅助面板", "Toggle inspector")} aria-pressed={!!inspector} title={tr("笔记关联与 AI 助手", "Note links & AI assistant")}><Icon name="panelRight" size={17}/></IconButton></header>
      ...
      <div className="notes-view" hidden={!!centerView}>
        {session ? <><div className="document-toolbar"><span className="document-path" title={session.path}><Icon name="note" size={14}/>{session.path.replace(/\.md$/i, "")}</span><div className="view-switch" aria-label={tr("笔记视图", "Note view")}>{(["source", "live", "preview", "split"] as const).map(mode => <button type="button" key={mode} aria-pressed={s.editorMode === mode} onClick={() => s.setPreferences({editorMode:mode})} title={mode === "source" ? tr("编辑", "Edit") : mode === "live" ? tr("实时预览", "Live preview") : mode === "preview" ? tr("预览", "Preview") : tr("分屏", "Split")}><Icon name={mode === "source" ? "edit" : mode === "live" ? "preview" : mode} size={14}/><span>{mode === "source" ? tr("编辑", "Edit") : mode === "live" ? tr("实时", "Live") : mode === "preview" ? tr("预览", "Preview") : tr("分屏", "Split")}</span></button>)}</div></div>
        ...
        {Object.values(s.sessions).map(doc => <div key={doc.path} hidden={doc.path !== s.activePath} className={`note-surface mode-${s.editorMode}`} style={{"--split-ratio": `${s.splitRatio}%`} as CSSProperties}><section className="editor-panel" hidden={s.editorMode === "preview"}><EditorPane session={doc} theme={s.theme} readOnly={s.workspaceFrozen || s.vaultStale} onChange={value => s.updateContent(doc.path, value)} onSave={() => {timers.current.get(doc.path)?.cancel(); void s.save(doc.path, "manual");}} onUploadFiles={apiReady ? (files, source) => void uploadFiles(files, doc.path, undefined, source) : undefined} onRegisterCaretInsert={handler => s.registerCaretInsert(handler ? (path, markdown) => (path === doc.path ? handler(markdown) : false) : null)}
      livePreview={s.editorMode === "live" ? { notePath: doc.path, resolveResourceUrl: (path) => vaultResourceUrl(path), onOpenLink: (target, kind) => { if (kind === "wikilink") openWikilink(target); else void s.openFile(target).catch(() => undefined); } } : null} attachmentBusy={s.attachment.busy} attachmentHint={tr("拖到此处或粘贴图片", "Drop here or paste an image")}/></section><div hidden={s.editorMode !== "split"} className="split-divider"><PaneResize value={s.splitRatio} onChange={s.setSplitRatio} percent label={tr("调整编辑与预览比例", "Resize editor and preview")}/></div><section className="preview-panel" hidden={s.editorMode === "source" || s.editorMode === "live"}><PreviewPane source={doc.content} notePath={doc.path} resolveResourceUrl={vaultResourceUrl} wikilinkExists={wikilinkExists} onOpenWikilink={openWikilink} attachmentBusy={s.attachment.busy} disabled={s.workspaceFrozen || s.vaultStale} onDropFiles={apiReady ? files => void uploadFiles(files, doc.path, undefined, "preview-drop") : undefined}/></section></div>)}
      </div>
    </main>
