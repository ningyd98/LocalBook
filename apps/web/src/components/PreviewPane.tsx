...
export function PreviewPane({ source, notePath, resolveResourceUrl, onDropFiles, attachmentBusy = false, disabled = false, wikilinkExists, onOpenWikilink, onToggleTask }: { source: string; notePath?: string | null; resolveResourceUrl?: (path: string) => string; onDropFiles?: (files: File[]) => void; attachmentBusy?: boolean; disabled?: boolean; wikilinkExists?: (target: string) => boolean; onOpenWikilink?: (target: string) => void; onToggleTask?: (index: number) => void }) {
  const { tr } = useI18n();
  const frontmatter = /^\uFEFF?---\r?\n([\s\S]*?)\r?\n(?:---|\.\.\.)(?:\r?\n|$)/.exec(source);
